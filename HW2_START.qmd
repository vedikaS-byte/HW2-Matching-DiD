...
```{r}
# Set a seed for reproducible matching
#set.seed(123)
...
*Response:* There is a mismatch in the number and size of the precincts between the surveyed years, so there must be a correct match that adjusts for this discrepancy between years.
...
wind_clean <- wind %>% select(proposed_turbine_3km, log_home_val_07, p_uni_degree, log_median_inc, log_pop_denc)
...
*Response:* The `log_pop_denc` appears to be the most imbalanced covariate. The `log_median_inc` is the most balanced covariate between years; there is significant overlap in the distribution of precincts between the two groups.

**2B.Q2** Describe in your own words why these covariates might be expected to confound the treatment estimate:

*Response (2-4 sentences):* \*\* More populated == more away bc do not want site disturbance??

------------------------------------------------------------------------

**2B.Q3** Intuition check: What type of data do you need to conduct a matching analysis?

*Response:* A matching analysis would require at least significantly more observations for the control group compared to the treatment group, as well as pre-treatment covariates; covariates prior to the analysis are expected to influence the effect on the outcome (ie. the treated group).
...
*Response:* The adjusted standardized mean differences are within the defined threshold in the love plot.
...
*Response:* The match model is better because the standardized mean differences are closer to 0 within the defined interval.
...
**2F.Q1** Have you identified a causal estimate using this approach: Why or why not?\*\* (is it causal if it is observational??)

*Response:* Yes, there is a causal estimate using this approach, as the matched data best controls for the effect of the proposed turbine distance on the change in incumbent voter share (p \< .05) after having adjusted for the effect of the covariates in the original data set.

**2F.Q2** When using a matching method, what is the main threat to causal identification?

*Response:* It is important to consider that matching by itself does not directly resolve the selection on unobservables in the data; there may be underlying selection bias or additional confounding influence on the outcome excluded from the data.

Matching alone does not solve the selection on unobservables problem
...
*Response:* \*\* Only the treatment group is considered in the regression of the change in incumbent voter share; the ATT would only observe those in the treatment group for this model (ie. precincts within 3 km of a wind turbine).

Only looking at the effect of the controls on the treatment group from the subset

ATE: entire population

ATT: only those in treatment group are measures
...
*Response (4-6 sentences):* \*\*\* Compare 2003-2007 and 2007-2011. In the pre-treatment period (2003-2007), there appears to be a parallel trend for operational tubines compared to no wind turbines in operation for the effect of the liberal party voter share. However, this parallel trend is not observed for prescincts for proposed turbines; there does not seem to be a significant relationship between proposed turbines and no turbines in prescincts on the liberal party voter share between 2003-2007. In the post-treatment period (2007-2011), there does not seem to be a parellel trend between operational turbines and no turbines on the liberal party voter share; the parallel trends assumption fail for the outcome. The similar pattern is observed for proposed turbines and no turbines in precincts following the post-treatment period. require more points....
...
model2_ff <- lm(perc_lib ~ operational_turbine + precinct_id + year, data = sample_40_precincts)

summ(model = model1_ff, model.fit = F, robust = T)
summ(model = model2_ff, model.fit = F, robust = T)
```
...
model2_ff <- feols(perc_lib ~ proposed_turbine | precinct_id + year, data = sample_40_precincts, cluster = ~precinct_id)
