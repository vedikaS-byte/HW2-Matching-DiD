...
*Response:* The standard error estimates changed; the standard error estimates increased from the original model. In particular, the standard error for the `proposed_turbine` coefficient increased from 0.031 to 0.039 and is now statistically significant (p \< .05). The robust standard errors relax the assumption of constant variance of errors and account for heteroskedasticity in the residuals. Because heteroskedasticity is likely present in the original model, the standard errors underestimated uncertainty while the robust adjustment provides more reliable (larger) standard errors.
